...
const canBlockInterpret = pairs >= TARGET_PAIRS_FOR_INTERPRET;
  ...
  try {
    const kickoffInstruction =
      'Начни анализ с вопроса о самом ярком, эмоционально заряженном или необычном образе из ЭТОГО КОНКРЕТНОГО ФРАГМЕНТА сна. ' +
      'Используй один из типов вопросов: "Что для тебя значит [образ]?", "Какие ощущения вызывает [образ]?", "Что ты чувствовал, когда [действие]?" или "Что происходит с тобой, когда [событие]?". ' +
      'Один простой вопрос без преамбулы, без оценок, без готовой интерпретации. ' +
      'Не используй элементы, которых нет в этом фрагменте. ' +
      'Не используй "Почему?" для характеристик (старая, синий), используй "Что для тебя означает..." или "С чем у тебя связан..."';
