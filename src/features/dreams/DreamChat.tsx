...
const canBlockInterpret = pairs >= TARGET_PAIRS_FOR_INTERPRET;
  ...
  try {
    const kickoffInstruction =
      'Начни анализ с вопроса о самом ярком или необычном образе из этого фрагмента сна. ' +
  'Спроси открытым вопросом, что этот образ значит для человека. ' +
  'Например: "Что для тебя значит [образ]?" или "Какие ощущения вызывает [образ]?" ' +
  'Один простой вопрос без преамбулы, который не содержит готовой интерпретации.';
