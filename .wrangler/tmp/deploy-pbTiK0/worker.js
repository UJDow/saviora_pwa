...
function buildCorsHeaders(origin) {
  ...
  if (allowedOrigins.includes(norm)) {
    return {
      ...
      "Vary": "Origin"
      // Добавлено для поддержки куки, если потребуется
...
var worker_default = {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/dreams" && request.method === "GET") {
      ...
      try {
        const d1 = env.DREAMS_D1;
        const result = await d1.prepare("SELECT * FROM dreams WHERE user = ? ORDER BY date DESC", [userEmail]).all();
    ...
    if (url.pathname === "/dreams" && request.method === "POST") {
      ...
      try {
        const d1 = env.DREAMS_D1;
        await d1.prepare(
          `INSERT INTO dreams (id, user, dreamText, title, blocks, globalFinalInterpretation, dreamSummary, category, date, similarArtworks, context)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            id,
            userEmail,
        ...
        const dream = { id, user: userEmail, dreamText, title, blocks, globalFinalInterpretation, dreamSummary, category, date, similarArtworks, context };
    ...
    if (url.pathname.startsWith("/dreams/") && request.method === "DELETE") {
      ...
      try {
        const d1 = env.DREAMS_D1;
        const result = await d1.prepare("DELETE FROM dreams WHERE id = ? AND user = ?", [id, userEmail]).run();
        if (result.changes === 0) {
    ...
    if (url.pathname.startsWith("/dreams/") && request.method === "PUT") {
      ...
      try {
        ...
        const result = await d1.prepare(
          `UPDATE dreams SET dreamText = ?, title = ?, blocks = ?, globalFinalInterpretation = ?, dreamSummary = ?, category = ?, date = ?, similarArtworks = ?, context = ? WHERE id = ? AND user = ?`,
          [
            dreamText,
            title,
            JSON.stringify(blocks),
        ...
        if (result.changes === 0) {
          return new Response(JSON.stringify({ error: "Dream not found or no changes made" }), {
            status: 404,
            headers: { "Content-Type": "application/json", ...corsHeaders }
          });
        }
